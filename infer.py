...
from models.encoders.resnet import resnet50, wide_resnet50_2, resnet18, resnet34
...
def evaluation(encoder, bn, decoder, dataloader, device, classes, amap_mode="add"):
    ...
    for cls in classes:
        ...
        aupro_list[cls] = []
    with torch.no_grad():
        for img, gt, cls, cls_id in tqdm(dataloader):
            cls = cls[0]
            img = img.to(device)
            inputs = encoder(img)
            ...
            # print(anomaly_map.shape, gt.shape)
            gt[gt > 0.5] = 1
            gt[gt <= 0.5] = 0
            # 全黑图片没办法计算aupro
            if gt.max() != 0:
    ...
    return auroc_sp, auroc_px, aupro_list


def evaluation_pro(encoder, bn, decoder, dataloader, device, classes, amap_mode="add"):
    encoder.eval()
    bn.eval()
    decoder.eval()
    pro_sum = 0
    ...
    with torch.no_grad():
        for img, gt, cls, cls_id in tqdm(dataloader):
            ...
            gt[gt > 0.5] = 1
            gt[gt <= 0.5] = 0
            pre[cls].append(anomaly_map)
            gt = gt.squeeze([0, 1]).cpu().numpy().astype(int)
            mask[cls].append(gt)
    for cls in classes:
        pro_sum += compute_pro(np.array(mask[cls]), np.array(pre[cls]))

    return pro_sum / len(classes)
...
def infer(checkpoint_path):
    ...
    test_data = Uni_MVTecDataset(
        img_size=256, root="/data01/fkw/datasets/mvtec", mode="test"
    )
    ...
    auroc_sp, auroc_px, aupro_px = evaluation(
        encoder,
        bn,
        decoder,
        test_dataloader,
...
infer(
    "end/2arcosine+decoder+0.005/best_model.pth",
)
